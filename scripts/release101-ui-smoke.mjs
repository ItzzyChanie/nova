...
const edge = spawn(
  ...
  [
    ...
    `--user-data-dir=${path.join(root, ".tmp/release101-edge")}`,
    "about:blank",
  ],
...
try {
  ...
  for (const size of [
    ...
    await evaluate(
      `([...document.querySelectorAll('nav button')].find(b=>b.textContent.trim()==='Voice & wake word')).click()`,
    );
    await pause(250);
    const layout = await evaluate(
      `(()=>{const d=document.documentElement,s=document.querySelector('.sidebar');return {outside:[...document.querySelectorAll('body *')].filter(e=>e.getBoundingClientRect().bottom>innerHeight+1).slice(0,8).map(e=>({tag:e.tagName,cls:e.className,bottom:e.getBoundingClientRect().bottom,position:getComputedStyle(e).position,overflow:getComputedStyle(e).overflow})),dimensions:[d.scrollHeight,d.clientHeight,d.scrollWidth,d.clientWidth,document.body.getBoundingClientRect().height],documentOverflow:d.scrollHeight>d.clientHeight||d.scrollWidth>d.clientWidth,sidebarOverflow:s.scrollHeight>s.clientHeight,shellHeight:document.querySelector('.app-shell').getBoundingClientRect().height,viewport:innerHeight,styles:[...document.styleSheets].map(s=>s.href)}})()`,
    );
    if (
      layout.documentOverflow ||
      layout.sidebarOverflow ||
      Math.abs(layout.shellHeight - size.height) > 1
    )
      throw new Error("Dashboard overflow: " + JSON.stringify(layout));
    layoutChecks.push({ size, ...layout });
  }
  fs.writeFileSync(
    ".tmp/release101-voice.png",
    Buffer.from(
  ...
  await cdp("Page.navigate", { url: "http://127.0.0.1:1423/assistant.html" });
  await pause(600);
  const assistant = await evaluate(
    `(()=>{const p=document.querySelector('.assistant-panel'),d=document.documentElement;return {panel:!!p,rootBackground:getComputedStyle(d).backgroundColor,minimumBodyWidth:getComputedStyle(document.body).minWidth,overflow:d.scrollWidth>d.clientWidth||d.scrollHeight>d.clientHeight,panelHeight:p?.getBoundingClientRect().height,styles:[...document.styleSheets].map(s=>s.href)}})()`,
  );
  if (
    !assistant.panel ||
    assistant.overflow ||
    assistant.minimumBodyWidth !== "0px" ||
    assistant.styles.some((s) => s?.includes("/main-"))
  )
    throw new Error("Assistant isolation failed: " + JSON.stringify(assistant));
  await evaluate(
    `(()=>{const p=document.querySelector('.assistant-panel');p.dataset.state='listening';p.dataset.speaking='true';document.querySelector('.voice-core').innerHTML='<div class="voice-equalizer">'+[0,1,2,3].map(i=>'<span style="--bar:'+i+'"></span>').join('')+'</div>';document.querySelector('.voice-orb').style.setProperty('--voice-level','0.7');document.querySelector('h1').textContent='Listening...';})()`,
  );
  const animations = await evaluate(
    `({ring:getComputedStyle(document.querySelector('.voice-orb'),'::before').animationName,bars:getComputedStyle(document.querySelector('.voice-equalizer span')).animationName})`,
  );
  if (animations.ring !== "voice-ring" || animations.bars !== "equalize")
    throw new Error("Assistant animation missing");
  fs.writeFileSync(
    ".tmp/release101-assistant.png",
    Buffer.from(
      (await cdp("Page.captureScreenshot", { format: "png" })).data,
      "base64",
    ),
  ...
  await evaluate(
    `document.querySelector('.assistant-panel').dataset.processing='true'`,
  );
  const border = await evaluate(
    `getComputedStyle(document.querySelector('.assistant-panel'),'::before').animationName`,
  );
  if (border !== "processing-border")
    throw new Error("Processing border missing");
  await evaluate(
    `document.querySelector('h1').textContent='Your approved project has started. '.repeat(10)`,
  );
  const expanded = await evaluate(
    `document.querySelector('.assistant-panel').getBoundingClientRect().height`,
  );
  if (expanded <= 180) throw new Error("Assistant content cannot expand");
  await cdp("Emulation.setDeviceMetricsOverride", {
    width: 520,
    height: Math.ceil(expanded),
    deviceScaleFactor: 1,
    mobile: false,
  });
  if (
    ...
    throw new Error("Expanded assistant overflow");
  fs.writeFileSync(
    ".tmp/release101-layout-results.json",
    JSON.stringify(
      {
        layoutChecks,
        assistant,
        ...
        scope:
          "Production browser assets; animation states supplied by the test, not live audio/native resize IPC",
      },
      null,
      2,
    ),
  ...
  fs.writeFileSync(
    ".tmp/release101-ui-results.json",
    JSON.stringify(
      {
        pages: results,
